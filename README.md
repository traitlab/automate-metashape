...
Repeat `--task` to chain steps in one open/save cycle:
...
Note this writes no processing log, so a product built this way has no provenance record of its own — use `main.py` for real missions. The `--create` and `add_photos` tasks exist for debugging only; they skip the mission_id convention, the mission lock, and the log.
